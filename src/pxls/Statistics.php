...
class Statistics {
    ...
    public function pixelsPlaced($time=0) {
        if($time == 0) {
            return $this->db->query("SELECT COUNT(id) AS total FROM pixels WHERE NOT rollback_action AND NOT mod_action")->fetch(\PDO::FETCH_OBJ)->total;
        } else {
            $time = date("Y-m-d H:i:s",strtotime($time));
            $pixels = $this->db->prepare("SELECT COUNT(id) AS total FROM pixels WHERE NOT rollback_action AND NOT mod_action AND time > :time");
